...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" "ansible_hosts"{
    inline = [
      ...
      "echo \"${join("\n",format("%s ansible_ssh_host=%s", openstack_compute_instance_v2.consul.0.name, openstack_compute_instance_v2.consul.0.access_ip_v4))}\" >> /ansible/environments/dev/hosts",
